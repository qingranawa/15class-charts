...
**票 (Vote)** — 用户对条目的投票记录，每人每天每条目仅可投一次（以 `created_at` 日期判断）。→ `schema.sql` votes 表
**赞 / 踩** — 投票方向：赞消耗 1 票（value = 1），踩不消耗票数（value = -1）。→ `functions/api/vote.js`
**分数 (Score)** — 条目得分 = MAX(0, MIN(10, 赞数 - 踩数))，0-10 封顶。→ `entries.score`
**票数余额 (Vote Balance)** — 用户剩余可用票数，初始 10 票，每日 +1（上限 10）。只有赞消耗票数，踩不消耗。→ `users.vote_balance`
...
**账户 tab** — 取代旧"我上传的"tab，聚合账户信息、改密、我上传的人物管理。→ `index.html` tabAccount
